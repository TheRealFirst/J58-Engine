...
b8 platform_startup(
    ...
    if (xcb_connection_has_error(state->connection)) {
        JFATAL("Failed to connect to X server via XCB.");
        return FALSE;
    }
    ...
    if (stream_result <= 0) {
        JFATAL("An error occurred when flusing the stream: %d", stream_result);
        return FALSE;
    }
...
void platform_get_required_extension_names(const char*** names_darray){
    darray_push(*names_darray, &"VK_KHR_xcb_surface"); // VK_KHR_xcb_surface?
}
